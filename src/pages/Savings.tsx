...
import { toast } from 'sonner';
import { cn } from '@/lib/utils';
import { savingsService } from '@/lib/appwrite'; // Import Appwrite savings service
...
const Savings = () => {
  ...
  // Fetch savings data from Appwrite
  useEffect(() => {
    const fetchSavings = async () => {
      setIsLoading(true);
      try {
    ...
    // Fetch users for admin (if applicable)
    if (user?.isAdmin) {
      const fetchUsers = async () => {
        try {
          const usersData = await savingsService.getAllSavings(); // Replace with actual method to fetch users
  ...
  const handleSavingsSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create savings
      await savingsService.createSavings(newSaving.userId, newSaving.amount, newSaving.description);
